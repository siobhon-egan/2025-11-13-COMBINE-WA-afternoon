...
```{r}
readsumsdf = data.frame(nreads = sort(taxa_sums(pseq), TRUE), sorted = 1:ntaxa(pseq), 
    type = "OTUs")
readsumsdf = rbind(readsumsdf, data.frame(nreads = sort(sample_sums(pseq), 
    TRUE), sorted = 1:nsamples(pseq), type = "Samples"))
